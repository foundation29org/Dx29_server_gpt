...
function encryptSubscriptionKey(data) {
  ...
  // Intentar usar la función antigua primero (para compatibilidad)
  if (crypto.createCipher) {
    // Node.js versión antigua
    var cipher = crypto.createCipher('aes-256-ecb', ENCRYPTION_KEY);
    return cipher.update(data, 'utf8', 'hex') + cipher.final('hex');
  } else {
    // Node.js versión nueva - simular comportamiento de createCipher
    const key = Buffer.from(ENCRYPTION_KEY.padEnd(32, '0').slice(0, 32));
    const iv = crypto.createHash('md5').update(ENCRYPTION_KEY).digest().slice(0, 16);
...
function decryptSubscriptionKey(data) {
  ...
  if (crypto.createDecipher) {
    ...
    // Node.js versión nueva - simular comportamiento de createDecipher
    const key = Buffer.from(ENCRYPTION_KEY.padEnd(32, '0').slice(0, 32));
    const iv = crypto.createHash('md5').update(ENCRYPTION_KEY).digest().slice(0, 16);
